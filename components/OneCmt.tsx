...
const OneCmt: React.FC<OneCmtProps> = ({
  comment,
    ...
    const checkLikeStatus = async () => {
      try {
        ...
        if (Array.isArray(response)) {
          const isInLikes = response.some(
            (like) => like.commentId?.toString() === comment.id.toString()
          );
  ...
  const handleSaveEdit = async () => {
    try {
      await updateComment(comment.id.toString(), comment.recipeId.toString(), {
  ...
  return (
    <>
          ...
          <View>
            ...
            <View style={{ flexDirection: "row", marginTop: 8, gap: 8 }}>
              <TouchableOpacity
                ...
                style={[
                  styles.editButton,
                  { backgroundColor: "#FF5D00", borderRadius: 6 },
                ]}
              >
                <Text
                  style={{
      ...
      <ReportModal
        ...
        reportedItemId={comment.id} // Thêm ID của bình luận
      />
