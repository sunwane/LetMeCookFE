...
import {
  CommentItem,
  countCommentByAccountId,
} from "@/services/types/CommentItem";
import {
  getRecipeCountByUserAPI,
  getUserInfoAPI,
  uploadAvatarAPI,
} from "@/services/types/UserInfo";
    ...
    const fetchCurrentUser = async () => {
      try {
        ...
        const accountItem: AccountItem = {
          ...
          email: userEmail,
          userName:
            userEmail.split("@")[0] || `User_${userInfo.id.substring(0, 8)}`,
          avatar: userInfo.avatar,
          status: "ACTIVE",
          ...
          userBirthday: new Date(
            Date.now() - (userInfo.age || 25) * 365.25 * 24 * 60 * 60 * 1000
  ...
  useEffect(() => {
    const fetchActivityCount = async () => {
      ...
      try {
        ...
        const count = response ?? 0; // Sử dụng trực tiếp response làm số lượng
