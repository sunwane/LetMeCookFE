...
export const getFavouriteRecipeByAccount = async(): Promise<ApiResponse<RecipeItem[]>> => {
  const token = await getAuthToken();
  const headers: HeadersInit = {
    'Content-Type': 'application/json',
  };
  ...
  const response = await fetch(`${API_BASE_URL}/recipe/getFavouriteRecipeByAccount`, {
    method: 'GET',
    headers,
  });
...
export const getRecipeById = async (recipeId: string): Promise<ApiResponse<RecipeItem>> => {
  const token = await getAuthToken();
  const headers: HeadersInit = {
    'Content-Type': 'application/json',
  };
  ...
  const response = await fetch(`${API_BASE_URL}/recipe/getRecipeById/${recipeId}`, {
    method: 'GET',
    headers,
  });
